...
const { validateBookExist, validateBookFound, validateBookLend, validateBookLendDelete, validatePersonExist, validateCategoryExist} = require("../validations");
...
router.post('/libro', async (req, res) => {
    let id = req.body.id, nombre = req.body.nombre, categoria = req.body.categoria_id;
    if(!id || id.trim() == "") {
    ...
    try {
        ...
        db.query("INSERT INTO libro (nombre,descripcion,categoria_id,persona_id) values (?,?,?,?)", [req.body.nombre, req.body.descripcion, req.body.categoria_id, req.body.categoria_id], (error, registro, campos) => {
            if (error) {
